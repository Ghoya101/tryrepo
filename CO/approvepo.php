...
<input class="nav-proj" type=button value="Purchasing" Onclick="window.location.href='index.php'">
<!--PO INFO TABLE-->
<br	>
<table class="po-info">
<?php if($ctr1 == 0 ){?>
...
<td ROWSPAN=2><?php echo $TERMS.' days';?></td>
</tr>
...
$t = ($row['POQTY'])* ($row['POUNITPRICE']);
$AMOUNT += $t;



/*
$q2 = "SELECT * FROM `sms_pocontent` WHERE ID = '$id' ";
...
<td style="width:15%;"><?php echo number_format($t,2);?></td>
</tr>
...
<input type=hidden name="amp" value="<?php echo $AMOUNT;?>">
...
<?php
$query  = "SELECT * FROM sms_projpo a inner join sms_projects b ON a.PROJID = b.PROJID WHERE POID = '$PO';";
$run	= $con->query($query);
$ctr	= $run->num_rows;
if($ctr!=0)
{
...
<td  style="width:40%;height:50px;background:white;">
<select name="subcat" style="width:50%;float:left;" id="subcat1">
...
?>
<option style="background:#dde7f7;font-weight:bold;padding:2px;" value="<?php echo '';?>"><?php echo $cat.'   '. $cdesc;?></option>
<?php
...
$catp = $_GET['catproj'];
$pos = $_GET['poid'];
$aq = "SELECT * FROM sms_subcat WHERE ID = '$val' ;";
$runaq = $con->query($aq);
while($getaq = $runaq->fetch_assoc()){$cat1 = $getaq['CATID'];$cat2=$getaq['SUBCATID'];$Cd = $getaq['DESCRIPTION'];}
 $que = "SELECT * FROM sms_pocontent WHERE POID = '$pos' ";
 $ru = $con->query($que);
 $ar = 0;
 while($ge= $ru->fetch_assoc()){$ram = ($ge['POQTY'])* ($ge['POUNITPRICE']);$ar	+= $ram;;}

$ques = "SELECT * FROM sms_subsubcat WHERE cat1Id = '$cat1' AND cat2Id = '$cat2' AND PROJID = '$catp'  ;";
$resq = $con->query($ques);
$ctrques = $resq->num_rows;
while($geton =$resq->fetch_assoc()){$tot = floatval($geton['AMOUNT']);}
$costp = $tot-floatval($ar);
if($costp > 0 ){$style="width:50%;height:100%;float:right;color:green;";}else{$style="width:50%;height:100%;float:right;color:red;";}
$costp = number_format($costp,2);
$costp = 'if approve under category '.$Cd.' balance will be Php'.$costp;
$valme = $val;
}?>
<div id="log" style="<?php echo $style;?>">
<input type=hidden name="valme" value="<?php echo $valme;?>" id="">
<?php echo $costp;?>
</div>
</td>
...
-->
		<input type=hidden name="po-id" value="<?php echo $PO;?>" id="setpo">
...
<script>
var elem = document.getElementById("subcat1");

elem.onchange = function(){
    var hiddenDiv = document.getElementById("log");
    //hiddenDiv.style.display = (this.value == "") ? "none":"block";
	var valme = this.value;
//	var am  = document.getElementById("amp").value;
	var po  = document.getElementById("setpo").value;
	var catproj = po[0];
	//document.getElementById("log").innerHTML = "<b>Previous: </b>"+valme;
	//var a = "approvepo.php?poid="+po+"&&valme="+valme+"&&ampo="+am+"&&catproj="+catproj;
	var a = "approvepo.php?poid="+po+"&&valme="+valme+"&&catproj="+catproj;
	//document.getElementById("log").innerHTML = a;
	if (valme== "")
	{
	document.getElementById("log").innerHTML = "Choose a valid category.";
	}else{
